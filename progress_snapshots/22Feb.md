...
Manual stack setup and jump from assembly to C.
VGA text output from C showing a success message.
...
### What's Still Undone
Linker still warns about an RWX load segment (`linker.ld` needs PHDR/section permission cleanup).
...
No memory manager.
No syscall ABI, no user mode, no process model.
